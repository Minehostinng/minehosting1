// api/index.js - Main entry point for all requests
...
console.log('[Init] GitHub OAuth:', { GITHUB_CLIENT_ID: !!GITHUB_CLIENT_ID, CALLBACK_URL });
...
passport.use(new GitHubStrategy({
  ...
  try {
    ...
    console.log(`[Auth] User: ${userData.login}`);
...
app.get('/auth/github/callback',
  ...
  async (req, res) => {
    try {
      const user = req.user;
      console.log(`[Callback] Creating repo for ${user.login}`);
      ...
      if (repoReady) {
        try {
          ...
          console.log(`[!] Codespace error:`, csError.response?.data?.message);
        }
      }
